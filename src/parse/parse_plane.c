...
/*   Updated: 2025/05/12 12:33:19 by nlewicki         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	parse_plane(char **tokens, t_scene *scene)
{
	...
	plane.checker = false;
	init_texture(&plane.texture);
	if (tokens[4] && tokens[4][0])
		plane.texture.path = ft_strdup(tokens[4]);
	if (tokens[5]) {
	...
	scene->planes->ks = KS;
	// Debug print
	printf("Plane added: position=(%.2f, %.2f, %.2f), orientation=(%.2f, %.2f, %.2f), color=(%d, %d, %d), texture=%s, limit_width=%.2f, limit_height=%.2f\n",
		plane.position.x, plane.position.y, plane.position.z,
		plane.orientation.x, plane.orientation.y, plane.orientation.z,
		plane.color.r, plane.color.g, plane.color.b,
		plane.texture.path ? plane.texture.path : "none",
